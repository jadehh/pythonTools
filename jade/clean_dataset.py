...
# 通过标注框来裁剪图片
def CutImagesWithBox(VocPath, savedir=None, use_chinese_name=True,
                     prototxt_path="/home/jade/Data/StaticDeepFreeze/ThirtyTypes.prototxt"):
    if savedir is None:
...
def SaveEraseMask(dir, savedir):
    ...
    for image_path in image_paths:
        ...
        cv2.imwrite(os.path.join(savedir, "image", GetLastDir(image_path)[:-4] + ".png"),
                    cv2.resize(cv2.imread(image_path), (JADE_RESIZE_SIZE, JADE_RESIZE_SIZE)))
...
def RestoreRandomMaskWithVOC(dir, savedir):
    ...
    for image_path in image_paths:
        ...
        imagename, shape, bboxes, labels, labels_text, difficult, truncated = ProcessXml(
            os.path.join(voc_xml_path, GetLastDir(image_path).split("_")[0] + ".xml"))
        ...
        for mask_path in random_mask_paths:
            ...
            cv2.imwrite(
                os.path.join(savedir, DIRECTORY_IMAGES,
                             GetLastDir(image_path)[:-4] + "_" + GetLastDir(mask_path)[:-4] + ".png"),
                orignal_image)
            shutil.copy(os.path.join(voc_xml_path, GetLastDir(image_path).split("_")[0] + ".xml"),
                        os.path.join(savedir, DIRECTORY_ANNOTATIONS,
                                     GetLastDir(image_path)[:-4] + "_" + GetLastDir(mask_path)[:-4] + ".xml"))
            cv2
            NoLinePrint("Save image ...", processbar)
...
def RestoreEraseMaskWithVOC(dir, savedir):
    ...
    for image_path in image_paths:
        ...
        imagename, shape, bboxes, labels, labels_text, difficult, truncated = ProcessXml(
            os.path.join(voc_xml_path, GetLastDir(image_path)[:-4] + ".xml"))
        for i in range(len(bboxes)):
            box = bboxes[i]
        ...
        for i in range(len(images)):
            ...
            for j in range(len(masks)):
                ...
                cv2.imwrite(
                    ...
                    image)
                shutil.copy(masks[j], os.path.join(save_path, "masks",
                                                   GetLastDir(xml)[:-4] + "_" + str(i) + "_" + str(j) + ".jpg"))
