...
def packSetup(args, exec_path, uuid):
    ...
    if "models" in os.listdir(exec_path):
        exec_name =  get_app_name(args) + "_setup-V" + args.app_version[:-2] + "-" + args.app_version[-1]
    else:
        exec_name =  get_app_name(args) + "_setup-V" + args.app_version[:-2] + "-" + args.app_version[-1] + "-Portable"


    issname = os.path.join(os.path.expanduser("~"), "Desktop",
                           "{}.iss".format(exec_name))
